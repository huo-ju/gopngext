...
func (e *encoder) writeTextChunk(textitems []*TextItem) {
	for _, item := range textitems {
		k := []byte(item.Key)
		v := []byte(item.Value)
		s := make([]byte, 0, len(k)+len(v)+1)
		s = append(s, k...)
		s = append(s, 0)
		s = append(s, v...)
		e.writeChunk(s, "tEXt")
...
func Encode(w io.Writer, m image.Image, textitems []*TextItem) error {
	var e Encoder
	return e.Encode(w, m, textitems)
}

// Encode writes the Image m to w in PNG format.
func (enc *Encoder) Encode(w io.Writer, m image.Image, textitems []*TextItem) error {
